...
void detectObjects(std::string object) {
    cv::Mat im;
    Mat input_blob;
    ...
    Size network_input_image_size = Size(255, 255);
    vector<string> output_layer_name;
    inputVideo.set(CAP_PROP_FPS, 10);
    ...
    while (inputVideo.grab()) {
        ...
        //resize(im, im, Size(255, 200), 0, 0, INTER_CUBIC);
//        cout << int(inputVideo.get(5)) << endl;
        ...
        for (int output_index = 0; output_index < network_output.size(); output_index++) {
            ...
            for (int detect_index = 0; detect_index < detections_in_layer; detect_index++) {
                ...
                if ((max_conf > confidence_threshold) && (classnames.at(max_class.x) == object)) {
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        if (select < '0' || select > '10') {
            ...
            char key;
            Size image_size = Size(100, 100);                                      //initialize image size
            cv::Mat img = cv::Mat::zeros(image_size, CV_8UC3) + CV_RGB(60, 60, 60); //initialize image
            cv::namedWindow("7825 Project");
            x = 0;
            y = 0;
            ...
            std::thread cam(detectObjects,"cell phone");
